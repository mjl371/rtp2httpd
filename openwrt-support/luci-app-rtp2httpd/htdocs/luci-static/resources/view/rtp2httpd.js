...
return view.extend({
  ...
  // Helper function to generate playlist.m3u URL with token
  getPlaylistUrl: function (section_id) {
    return Promise.all([
      uci.load("rtp2httpd"),
      fs.read("/etc/rtp2httpd.conf").catch(function () {
        return "";
      ...
      if (use_config_file === "1") {
        // Parse port, token, hostname from config file content
        var configContent = results[1];
        var portMatch = configContent.match(/^\s*\*\s+(\d+)\s*$/m);
        if (!portMatch) {
          // Try alternative format: hostname port
        ...
        // Get port, token, hostname from UCI config
        port = uci.get("rtp2httpd", section_id, "port") || "5140";
        token = uci.get("rtp2httpd", section_id, "r2h_token");
        hostname = uci.get("rtp2httpd", section_id, "hostname");
      }
      ...
      try {
        ...
        // Fallback if URL parsing fails
        var fallbackUrl = "http://" + targetHostname + ":" + port + "/playlist.m3u";
        if (token) {
          fallbackUrl += "?r2h-token=" + encodeURIComponent(token);
        }
        return fallbackUrl;
      ...
      if (
        ...
        playlistUrl = finalProtocol + "://" + finalHost;
      } else {
        playlistUrl = finalProtocol + "://" + finalHost + ":" + finalPort;
      }
  ...
  // Helper function to open playlist.m3u URL
  openPlaylist: function (section_id) {
    return this.getPlaylistUrl(section_id).then(function (url) {
      window.open(url, "_blank");
    });
  },

  // Helper function to copy playlist.m3u URL to clipboard
  copyPlaylistUrl: function (section_id) {
    return this.getPlaylistUrl(section_id).then(function (url) {
      if (navigator.clipboard && window.isSecureContext) {
        // Modern clipboard API
        return navigator.clipboard.writeText(url).then(function () {
          alert(_("Playlist URL copied to clipboard!"));
        }).catch(function (err) {
          alert(_("Failed to copy URL: ") + err);
        });
      } else {
  ...
  render: function () {
    ...
    // Playlist.m3u buttons
    o = s.taboption("player", form.Button, "_playlist_open", _("Open Playlist"));
    o.inputtitle = _("Open Playlist.m3u");
    o.inputstyle = "apply";
    o.onclick = function (ev, section_id) {
      return self.openPlaylist(section_id);
    };

    o = s.taboption("player", form.Button, "_playlist_copy", _("Copy Playlist URL"));
    o.inputtitle = _("Copy Playlist.m3u URL");
    o.inputstyle = "default";
    o.onclick = function (ev, section_id) {
      return self.copyPlaylistUrl(section_id);
    };
